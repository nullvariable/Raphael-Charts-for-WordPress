...
(function () {
    ...
    function VBarchart(paper, x, y, width, height, values, opts) {
        ...
        /**
         * I've edited this function to actually work with Raphael 2.0.2 - Doug 03-07-12
         * I added an option for label color as that seemed to also be missing.
         * @param labels
         * @param isBottom
         */
            ...
            if (opts.stacked) {
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = 0; j < (multi || 1); j++) {
                        ...
                        if (j == multi - 1) {
                            ...
                            L = paper.text(bars[i * (multi || 1) + j].x, y + height - barvgutter / 2, label).attr(chartinst.txtattr).attr({ fill: opts.labelcolor || "#000"})//.insertBefore(covers[i * (multi || 1) + j]);
                ...
                for (var i = 0; i < len; i++) {
                    for (var j = 0; j < (multi || 1); j++) {
                        ...
                        L = paper.text(bars[i * (multi || 1) + j].x, isBottom ? y + height - barvgutter / 2 : bars[i * (multi || 1) + j].y - 10, label).attr(chartinst.txtattr).attr({ fill: opts.labelcolor || "#000"})//.insertBefore(covers[i * (multi || 1) + j]);

                        //console.debug(label);
                        /*console.debug(bars[i * (multi || 1) + j].x)
                        console.debug(isBottom ? y + height - barvgutter / 2 : bars[i * (multi || 1) + j].y - 10);*/
